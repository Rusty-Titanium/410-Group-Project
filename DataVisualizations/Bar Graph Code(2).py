...
mean_prices = df.groupby('Sector')['Close'].mean()
mean_prices.sort_values(ascending=True).plot(kind='bar')

plt.title('Mean Stock Prices by Sector')
plt.xlabel('Sector')
plt.ylabel('Mean Stock Price')

plt.subplots_adjust(left=0.2, right=0.9, bottom=0.38, top=0.9, wspace=0.4)
plt.show()